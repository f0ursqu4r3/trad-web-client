...
await writeFile(
  ...
  releaseHtml({
    ...
    url: updatesUrl,
    image: `${publicOrigin}/test-brand/social-preview.png`,
    imageAlt: 'Trad managed-trading workspace updates',
  }),
)

let generated = 0
for (const release of releases.filter((entry) => entry.status === 'published')) {
  ...
  await writeFile(
    ...
    releaseHtml({
      ...
      url,
      image: `${publicOrigin}/update-previews/${release.version}.png`,
