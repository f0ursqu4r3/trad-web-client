...
function apply(distance: StopPriceDistance): void {
  ...
  model.value = stopPriceFromReference(reference.value, props.positionSide, distance)
}
...
<template>
  <div class="stop-price-presets">
    <div class="preset-heading">
      ...
      <span v-if="reference">{{ direction }} {{ formatExactDecimal(reference) }}</span>
    ...
    <div class="preset-buttons">
      <button
        ...
        :title="
          reference
            ? `Set the stop ${distance}% ${direction} the latest exchange trade. Hyperliquid still triggers it from mark price.`
            : 'A fresh exchange price is required.'
        "
